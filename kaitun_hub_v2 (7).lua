...
-- =====================================================
--  AUTO CHOOSE TEAM
--  Tự động chọn team "Pirates" khi vào game
--  Chạy ngay khi script load, trước tất cả hệ thống
-- =====================================================
    ...
    repeat task.wait()
        pcall(function()
            for _, v in pairs(getconnections(
                Players.LocalPlayer.PlayerGui["Main (minimal)"]
                    .ChooseTeam.Container[teamName]
                    .Frame.TextButton.Activated
            )) do
                v.Function()
            end
        end)
    until Players.LocalPlayer.Team ~= nil
    print("[Kaitun] ✓ AutoChooseTeam → " .. teamName)
end

-- ★ Gọi ngay lập tức trước tất cả hệ thống
task.spawn(AutoChooseTeam)
print("[Kaitun] ✓ AutoChooseTeam → Đang chọn team...")


-- =====================================================
--  AUTO HOP MỖI 60 PHÚT
--  Tự động hop server sau mỗi 60 phút để tránh bị kick
--  và cải thiện drop rate / server quality
-- =====================================================

local _autoHopEnabled  = true
local _hopIntervalSecs = 60 * 60  -- 60 phút

local function AutoHopTimer()
    while task.wait(_hopIntervalSecs) do
        if not _autoHopEnabled then continue end

        print("[Kaitun] ⏱ Auto Hop: đã đủ 60 phút → đang tìm server mới...")
        pcall(function()
            ...
            if #servers > 0 then
                ...
                print("[Kaitun] ✓ Auto Hop → Server:", target)
                for _ = 1, 36 do
                    TeleportService:TeleportToPlaceInstance(game.PlaceId, target, Players.LocalPlayer)
                    task.wait(0.1)
                end
            else
                print("[Kaitun] ✗ Auto Hop: Không tìm thấy server phù hợp, thử lại sau...")
            end
        end)
    end
end

-- ★ Khởi động loop hop chạy song song
task.spawn(AutoHopTimer)
print("[Kaitun] ✓ AutoHop → Online (hop sau 60 phút)")
...
-- Tìm mob theo tên.
-- Ưu tiên 1: Workspace.Enemies (mob đã spawn gần player)
-- Ưu tiên 2: ReplicatedStorage spawn folder (mob đang ở xa, chưa load vào WS)
--   → Nếu tìm thấy trong RS: tween player tới spawn pos, chờ mob xuất hiện trong WS
-- Lưu ý: mob chỉ tồn tại ở MỘT trong hai nơi tại cùng một thời điểm.
local function GetMobByName(target, travelTimeout)
    local name = typeof(target) == "Instance" and target.Name or target
    travelTimeout = travelTimeout or 10  -- giây chờ tối đa sau khi tween

    -- ── Bước 1: Tìm trong Workspace (mob đang ở gần) ──────────────────────
    for _, v in pairs(Workspace.Enemies:GetChildren()) do
    ...
    -- ── Bước 2: Tìm spawn position trong ReplicatedStorage ────────────────
    local spawnCF = GetSpawnCFrameFromRS(name)
    if not spawnCF then
        return nil  -- Không có trong WS lẫn RS → mob không tồn tại trên server này
    end

    -- Mob đang ở xa, chưa load vào Workspace → tween player tới spawn point
    if Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
        local dist = (spawnCF.Position - Player.Character.HumanoidRootPart.Position).Magnitude
        if dist > 50 then
            SetTask("SubTask", string.format("[GetMob] %s ở RS → tween tới spawn (%.0f studs)...", name, dist))
            topos(spawnCF)
            WaitTween(15)
            task.wait(0.5)  -- nhường frame để server stream mob vào WS
        end
    end

    -- ── Bước 3: Poll Workspace sau khi đã đến gần ─────────────────────────
    local t0 = tick()
    repeat
        for _, v in pairs(Workspace.Enemies:GetChildren()) do
            if v.Name == name and IsAlive(v) then
        ...
        task.wait(0.2)
    until tick() - t0 > travelTimeout

    -- Đã đến nơi nhưng mob vẫn chưa xuất hiện (có thể vừa bị kill hoặc chưa respawn)
    SetTask("SubTask", string.format("[GetMob] %s không xuất hiện sau %.0fs tại spawn point", name, travelTimeout))
    return nil
end
...
-- =====================================================
--  AUTO TRAVEL TO SEA 2 (kích hoạt khi đạt level 700)
--  Quy trình: TravelDressrosa → Key/Ice Door → Ice Admiral → Sea 2
--  Nếu không có Ice Admiral → HopServers tự động
-- =====================================================

local _sea2Done = false

local function AutoTravelSea2()
    if _sea2Done then return true end

    -- Nếu đã ở Sea 2 hoặc Sea 3 rồi thì đánh dấu xong
    local currentMap = workspace:GetAttribute("MAP")
    if currentMap == "Sea2" or currentMap == "Dungeons" or currentMap == "Sea3" then
        _sea2Done = true
        return true
    end

    SetTask("MainTask", "[Sea2] Chuẩn bị lên Sea 2 (cần level 700+)...")

    local CommF_ = ReplicatedStorage.Remotes.CommF_

    -- ── Bước 1: Kiểm tra / khởi động Dressrosa Quest ──────────────────────
    local ok1, progress = pcall(function()
        return CommF_:InvokeServer("DressrosaQuestProgress", "Dressrosa")
    end)

    if not ok1 then
        SetTask("SubTask", "[Sea2] Không lấy được quest progress, thử lại...")
        task.wait(2)
        return false
    end

    if progress == 0 then
        SetTask("SubTask", "[Sea2] Travel tới Dressrosa...")
        pcall(function() CommF_:InvokeServer("TravelDressrosa") end)
        task.wait(3)
        return false
    end

    -- ── Bước 2: Kiểm tra cửa Ice ──────────────────────────────────────────
    local doorCanCollide = false
    local doorExists = pcall(function()
        doorCanCollide = Workspace.Map.Ice.Door.CanCollide
    end)

    if not doorExists then
        -- Workspace.Map.Ice chưa load, tween tới khu vực Ice chờ
        SetTask("SubTask", "[Sea2] Đang tới khu vực Ice...")
        topos(CFrame.new(1345, 37, -1329))
        WaitTween(15)
        task.wait(1)
        return false
    end

    if doorCanCollide then
        -- Cửa đang đóng, cần Key
        if not CheckBackpack("Key") then
            SetTask("SubTask", "[Sea2] Lấy Key (Detective quest)...")
            pcall(function()
                CommF_:InvokeServer("DressrosaQuestProgress", "Detective")
            end)
            task.wait(1.5)
        else
            SetTask("SubTask", "[Sea2] Dùng Key mở cửa Ice...")
            pcall(function()
                EquipWeapon("Key")
                task.wait(0.3)
                local keyHandle = Player.Character
                    and Player.Character:FindFirstChild("Key")
                    and Player.Character.Key:FindFirstChild("Handle")
                if keyHandle then
                    firetouchinterest(keyHandle, Workspace.Map.Ice.Door, 0)
                    task.wait(0.1)
                    firetouchinterest(keyHandle, Workspace.Map.Ice.Door, 1)
                end
            end)
            task.wait(1)
        end
        ...
        -- Cửa mở → tìm và giết Ice Admiral
        local iceAdmiral = GetMobByName("Ice Admiral")

        if iceAdmiral then
            SetTask("SubTask", "[Sea2] Giết Ice Admiral...")
            KillMobByName("Ice Admiral")
            task.wait(0.5)

            -- Sau khi giết xong, tween tới điểm kích hoạt portal Sea 2
            SetTask("SubTask", "[Sea2] Tween tới cổng Sea 2...")
            topos(CFrame.new(1345, 37, -1329))
            WaitTween(15)
            task.wait(2)
        else
            -- Không có Ice Admiral trong server → hop server tìm server khác
            SetTask("SubTask", "[Sea2] Không tìm thấy Ice Admiral → Hop server...")
            HopServers()
            return false
        end
    end

    -- ── Kiểm tra kết quả: đã lên Sea 2 chưa ──────────────────────────────
    local newMap = workspace:GetAttribute("MAP")
    if newMap == "Sea2" or newMap == "Dungeons" or newMap == "Sea3" then
        _sea2Done = true
        SetTask("MainTask", "[Sea2] ✓ Đã lên Sea 2 thành công!")
        return true
    end
...
local function CheckSea(seaNum)
    ...
    if seaNum == 1 then return map == "Sea1"
    elseif seaNum == 2 then return map == "Sea2" or map == "Dungeons"
    elseif seaNum == 3 then return map == "Sea3"
    end
    return false
end

local function TeleportSea(seaNum)
    if CheckSea(seaNum) then return end
    SetTask("SubTask", "[AutoMelee] Travel tới Sea " .. seaNum .. "...")
    pcall(function()
        if seaNum == 1 then
...
-- =====================================================
--  SPECIAL MELEE CONDITION CHECKS
-- =====================================================

-- Cửa Phoeyu: CanCollide = false → cửa mở → có thể mua Death Step
local function CheckPhouyeDoor()
    local ok, result = pcall(function()
        return workspace.Map.IceCastle.Hall.LibraryDoor.PhoeyuDoor.CanCollide
    end)
    -- CanCollide = false → có thể đi qua → cửa "mở" → return true
    return ok and (result == false) or false
end

-- Water Key: cần để mua Sharkman Karate
local _waterKeyObtained = false
local function CheckWaterKey()
    if _waterKeyObtained then return true end
    local ok, bv = pcall(function()
        return ReplicatedStorage.Remotes.CommF_:InvokeServer("BuySharkmanKarate", true)
    end)
    if ok and (bv == 3 or bv == 1) then
        _waterKeyObtained = true; return true
    end
    local ok2, bw = pcall(function()
        return ReplicatedStorage.Remotes.CommF_:InvokeServer("BuySharkmanKarate")
    end)
    if ok2 and (bw == 0 or bw == 1 or bw == 2) then
        _waterKeyObtained = true; return true
    end
    return false
end
...
-- =====================================================
--  SEA 2 BOSS FARMING
--  Tide Keeper [Lv.1475] ưu tiên trước
--  Awakened Ice Admiral [Lv.1400] nếu không có Tide Keeper
--  Chỉ kích hoạt khi đang ở Sea 2
-- =====================================================

local function TryFarmSea2Bosses()
    if not CheckSea(2) then return false end

    -- Ưu tiên 1: Tide Keeper
    local tide = GetMobByName("Tide Keeper", 5)
    if tide then
        SetTask("MainTask", "[Sea2 Boss] Tide Keeper phát hiện → Farm (ưu tiên 1/2)")
        KillMobByName("Tide Keeper")
        return true
    end

    -- Ưu tiên 2: Awakened Ice Admiral
    local admiral = GetMobByName("Awakened Ice Admiral", 5)
    if admiral then
        SetTask("MainTask", "[Sea2 Boss] Awakened Ice Admiral phát hiện → Farm (ưu tiên 2/2)")
        KillMobByName("Awakened Ice Admiral")
        return true
    end
...
-- =====================================================
--  MASTERY FILE PERSISTENCE
--  File: <username>-main.js
--  Nội dung: {"BlackLeg: 400", "Electro: 300", ...}
--
--  Mục đích: Cache mastery đã đạt được → tránh re-buy
--  sau khi rejoin / tool tạm thời không có trong backpack
-- =====================================================

local _masteryFileName = Player.Name .. "-main.js"
local _masteryCache    = {}  -- { ["BlackLeg"] = 400, ["Electro"] = 300, ... }

local function SaveMasteryFile()
    pcall(function()
        local parts = {}
        for id, val in pairs(_masteryCache) do
            table.insert(parts, '"' .. id .. ': ' .. tostring(val) .. '"')
        end
        writefile(_masteryFileName, "{" .. table.concat(parts, ", ") .. "}")
    end)
end

local function LoadMasteryFile()
    pcall(function()
        if not isfile(_masteryFileName) then return end
        local content = readfile(_masteryFileName)
        -- Parse: {"BlackLeg: 400", "Electro: 300"}
        for id, val in content:gmatch('"(%w+):%s*(%d+)"') do
            _masteryCache[id] = tonumber(val)
        end
        print("[AutoMelee] ✓ Đã load mastery file:", _masteryFileName)
        for id, val in pairs(_masteryCache) do
            print(string.format("  %s: %d", id, val))
        end
    end)
end

-- Load ngay khi script khởi động
LoadMasteryFile()

-- Cập nhật cache và lưu file nếu có thay đổi
local function SetMasteryCache(meleeId, value)
    if (_masteryCache[meleeId] or -1) ~= value then
        _masteryCache[meleeId] = value
        SaveMasteryFile()
        print(string.format("[AutoMelee] 💾 Lưu mastery %s = %d → %s", meleeId, value, _masteryFileName))
    end
end


-- =====================================================
--  AUTO FULLY MELEE — HELPER FUNCTIONS
-- =====================================================
...
-- Đọc mastery trực tiếp từ backpack/character (không dùng cache)
-- Trả về số nếu có tool, nil nếu không có
local function ReadMasteryLive(meleeName)
...
-- Lấy mastery với cache-first để tránh xung đột:
--   1. Đọc live từ backpack
--   2. Update cache nếu live > cache (mastery tăng)
--   3. Trả về cache (luôn = giá trị cao nhất đã biết)
--   4. Nếu không có live VÀ không có cache → return nil (chưa sở hữu)
local function GetMeleeMastery(meleeId, meleeName)
    local live = ReadMasteryLive(meleeName)

    if live ~= nil then
        -- Có tool trong backpack: update cache nếu live cao hơn
        local cached = _masteryCache[meleeId]
        if not cached or live > cached then
            SetMasteryCache(meleeId, live)
        end
        -- Trả cache (không bao giờ thấp hơn giá trị đã đạt)
        return _masteryCache[meleeId]
    end

    -- Không có trong backpack → dùng cache (đã từng đạt mức nào đó)
    -- Điều này ngăn re-buy khi tool tạm thời không visible
    if _masteryCache[meleeId] then
        return _masteryCache[meleeId]
    end
...
-- Tween tới NPC, gọi remote mua melee, xác nhận bằng backpack
-- Trả về true nếu mua thành công
local function BuyMeleeFromNPC(meleeData)
    -- Teleport tới đúng sea trước
    if meleeData.requireSea then
        TeleportSea(meleeData.requireSea)
        task.wait(2)
    end

    local npcCFrame = GetNPCCFrame(meleeData.npc)
    if not npcCFrame then
        SetTask("SubTask", "[AutoMelee] ✗ NPC không tìm thấy: " .. meleeData.npc)
        return false
    end

    SetTask("MainTask", "[AutoMelee] ► Mua " .. meleeData.name)
    SetTask("SubTask",  "[AutoMelee] Tween → " .. meleeData.npc)

    DisableFastAttack()
    _G.CurrentTarget = nil

    -- Tween đến NPC, retry cho đến khi đủ gần (tối đa 60s)
    ...
    local id = meleeData.id

    -- ── Buy remote theo từng loại melee ────────────────────────────────────
    if id == "BlackLeg" then
        pcall(function() ReplicatedStorage.Remotes.CommF_:InvokeServer("BuyBlackLeg") end)

    elseif id == "Electro" then
        pcall(function() ReplicatedStorage.Remotes.CommF_:InvokeServer("BuyElectro") end)

    elseif id == "FishmanKarate" then
        pcall(function() ReplicatedStorage.Remotes.CommF_:InvokeServer("BuyFishmanKarate") end)

    elseif id == "DragonClaw" then
        pcall(function()
    ...
    elseif id == "Superhuman" then
        pcall(function() ReplicatedStorage.Remotes.CommF_:InvokeServer("BuySuperhuman") end)

    elseif id == "DeathStep" then
        -- Cần cửa Phoeyu mở (CanCollide = false)
        if not CheckPhouyeDoor() then
            SetTask("SubTask", "[AutoMelee] Death Step: cửa Phoeyu chưa mở, chờ Awakened Ice Admiral...")
            return false
        end
        pcall(function() ReplicatedStorage.Remotes.CommF_:InvokeServer("BuyDeathStep") end)

    elseif id == "SharkmanKarate" then
        -- Cần Water Key
        if not CheckWaterKey() then
            SetTask("SubTask", "[AutoMelee] Sharkman Karate: chưa có Water Key, chờ Tide Keeper...")
            return false
        end
        pcall(function() ReplicatedStorage.Remotes.CommF_:InvokeServer("BuySharkmanKarate") end)

    elseif id == "ElectricClaw" then
        pcall(function()
            local result = ReplicatedStorage.Remotes.CommF_:InvokeServer("BuyElectricClaw", true)
            if result == 4 then
                -- Đủ điều kiện → start quest + vào cửa
                ReplicatedStorage.Remotes.CommF_:InvokeServer("BuyElectricClaw", "Start")
                task.wait(1)
                ReplicatedStorage.Remotes.CommF_:InvokeServer(
                    "requestEntrance",
                    Vector3.new(-12463.874, 374.914, -7523.773)
                )
            end
        end)

    elseif id == "DragonTalon" then
        -- Đổi Bones nếu đủ 50
        pcall(function()
            local boneCount = ReplicatedStorage.Remotes.CommF_:InvokeServer("Bones", "Check")
            if type(boneCount) == "number" and boneCount >= 50 then
                ReplicatedStorage.Remotes.CommF_:InvokeServer("Bones", "Buy", 1, 1)
            end
        end)
        task.wait(0.5)
        pcall(function()
            local result = ReplicatedStorage.Remotes.CommF_:InvokeServer("BuyDragonTalon")
            if result == 3 then
                -- Cần dùng Fire Essence trong backpack
                if CheckBackpack("Fire Essence") then
                    local t0 = tick()
                    repeat
                        ReplicatedStorage.Remotes.CommF_:InvokeServer("BuyDragonTalon")
                        task.wait(1)
                    until not CheckBackpack("Fire Essence") or tick() - t0 > 30
                end
            end
        end)

    elseif id == "Godhuman" then
        pcall(function()
    ...
    -- ── Xác nhận mua thành công bằng ReadMasteryLive ────────────────────────
    local live = ReadMasteryLive(meleeData.name)
    if live ~= nil then
        SetMasteryCache(meleeData.id, live)
        SetTask("SubTask", "[AutoMelee] ✓ Mua thành công: " .. meleeData.name)
        EquipMelee(meleeData.name)
        return true
    end

    SetTask("SubTask", "[AutoMelee] ✗ Mua thất bại: " .. meleeData.name .. " (thử lại sau)")
    return false
end


-- =====================================================
--  AUTO FULLY MELEE — DANH SÁCH MELEE
--
--  Thứ tự cố định, có prereq rõ ràng:
--  Sea 1: BlackLeg(400) → Electro(400) → FishmanKarate(400)
--         → DragonClaw(400) → Superhuman(400)
--  Sea 2: DeathStep    (prereq: BlackLeg≥400, PhouyeDoor mở)
--         SharkmanKarate (prereq: FishmanKarate≥400, WaterKey)
--  Sea 3: ElectricClaw (prereq: Electro≥400)
--         DragonTalon   (prereq: DragonClaw≥400)
--         Godhuman      (prereq: tất cả ≥400)
-- =====================================================

local MeleesOrder = {
    -- ── Sea 1 ───────────────────────────────────────────────────────────
    {
        name = "Black Leg",       id = "BlackLeg",
        npc  = "Dark Step Teacher",
        reqMastery = 400,  price = {Beli = 150000},
        requireSea = 1,
    },
    {
        name = "Electro",         id = "Electro",
        npc  = "Mad Scientist",
        reqMastery = 400,  price = {Beli = 500000},
        requireSea = 1,
    },
    {
        name = "Fishman Karate",  id = "FishmanKarate",
        npc  = "Water Kung-fu Teacher",
        reqMastery = 400,  price = {Beli = 750000},
        requireSea = 1,
    },
    {
        name = "Dragon Claw",     id = "DragonClaw",
        npc  = "Sabi",
        reqMastery = 400,  price = {Fragments = 1500},
        requireSea = 1,
    },
    {
        name = "Superhuman",      id = "Superhuman",
        npc  = "Martial Arts Master",
        reqMastery = 400,  price = {Beli = 3000000},
        requireSea = 1,
    },
    -- ── Sea 2 ───────────────────────────────────────────────────────────
    {
        name = "Death Step",      id = "DeathStep",
        npc  = "Phoeyu, the Reformed",
        reqMastery = 400,  price = {Beli = 2500000, Fragments = 5000},
        requireSea = 2,
        -- prereq: BlackLeg đã đủ mastery 400
        prereq = function() return (_masteryCache["BlackLeg"] or 0) >= 400 end,
    },
    {
        name = "Sharkman Karate", id = "SharkmanKarate",
        npc  = "Sharkman Teacher",
        reqMastery = 400,  price = {Beli = 2500000, Fragments = 5000},
        requireSea = 2,
        -- prereq: FishmanKarate đã đủ mastery 400
        prereq = function() return (_masteryCache["FishmanKarate"] or 0) >= 400 end,
    },
    -- ── Sea 3 ───────────────────────────────────────────────────────────
    {
        name = "Electric Claw",   id = "ElectricClaw",
        npc  = "Previous Hero",
        reqMastery = 400,  price = {Beli = 2500000, Fragments = 5000},
        requireSea = 3,
        -- prereq: Electro đã đủ mastery 400
        prereq = function() return (_masteryCache["Electro"] or 0) >= 400 end,
    },
    {
        name = "Dragon Talon",    id = "DragonTalon",
        npc  = "Uzoth",
        reqMastery = 400,  price = {Beli = 2500000, Fragments = 5000},
        requireSea = 3,
        -- prereq: DragonClaw đã đủ mastery 400
        prereq = function() return (_masteryCache["DragonClaw"] or 0) >= 400 end,
    },
    {
        name = "Godhuman",        id = "Godhuman",
        npc  = "Ancient Monk",
        reqMastery = 400,  price = {Beli = 5000000, Fragments = 5000},
        requireSea = 3,
        -- prereq: tất cả các melee trước đều >= 400
        prereq = function()
            local needed = {
                "BlackLeg","Electro","FishmanKarate","DragonClaw","Superhuman",
                "DeathStep","SharkmanKarate","ElectricClaw","DragonTalon"
            }
            for _, id in pairs(needed) do
                if (_masteryCache[id] or 0) < 400 then return false end
            end
            return true
        end,
    },
...
-- =====================================================
--  AUTO FULLY MELEE — HÀM CHÍNH
--
--  Trả về:
--    "buying"  → đang tween/mua melee (skip farm tick này)
--    "mastery" → đang farm mastery (equip melee, farm bình thường)
--    false     → không có gì cần làm (chờ tiền / all done)
--
--  Fix xung đột buy loop:
--    - Cache-first: một khi biết mastery X → không bao giờ coi là nil
--    - Prereq rõ ràng: BlackLeg phải ≥400 trước khi mua DeathStep
--    - BuyMeleeFromNPC chỉ xác nhận thành công bằng ReadMasteryLive
--      → nếu fail, return false → outer loop thử lại chứ không advance
-- =====================================================
...
local function AutoFullyMelee()
    ...
    -- ── Ưu tiên farm boss Sea 2 mỗi khi chúng xuất hiện ──────────────────
    --   Tide Keeper trước → Awakened Ice Admiral sau
    if TryFarmSea2Bosses() then
        return "mastery"
    end

    local allDone = true

    for _, melee in ipairs(MeleesOrder) do

        -- ① Sea yêu cầu cao hơn sea hiện tại → dừng (chưa đến sea đó)
        if melee.requireSea and melee.requireSea > World then
            break
        end

        -- ② Prereq chưa đủ → dừng ở đây, farm melee prereq đó trước
        if melee.prereq and not melee.prereq() then
            allDone = false
            SetTask("SubTask", "[AutoMelee] Prereq chưa đủ cho " .. melee.name .. " → tiếp tục farm")
            break
        end

        -- ③ Lấy mastery (cache-first → không bao giờ re-buy nếu đã đạt)
        local mastery = GetMeleeMastery(melee.id, melee.name)

        if mastery == nil then
            -- Chưa sở hữu → kiểm tra tiền rồi mua
            allDone = false
            local canBuy, missCurrency, missAmount = CanAffordMelee(melee.price)

            if canBuy then
                local bought = BuyMeleeFromNPC(melee)
                if not bought then
                    -- Mua thất bại (door chưa mở, water key chưa có, v.v.)
                    -- Không advance, thử lại tick sau
                    return "buying"
                end
                -- Mua thành công → kiểm tra mastery mới
                mastery = GetMeleeMastery(melee.id, melee.name) or 0
                if mastery < melee.reqMastery then
                    EquipMelee(melee.name)
                    return "mastery"
                end
                -- mastery đã đủ ngay sau khi mua → tiếp tục vòng lặp
            else
                local need = missAmount - GetCurrency(missCurrency)
                SetTask("SubTask", string.format(
                    "[AutoMelee] Cần thêm %d %s để mua %s → tiếp tục farm",
                    need, missCurrency, melee.name
                ))
                break
            end

        elseif mastery < melee.reqMastery then
            -- Đã có nhưng mastery chưa đủ → equip và farm
            allDone = false
            SetTask("SubTask", string.format(
                "[AutoMelee] Farm mastery %s: %d / %d",
                melee.name, mastery, melee.reqMastery
            ))
            EquipMelee(melee.name)
            return "mastery"

        else
            -- mastery đủ → ghi cache (phòng trường hợp chưa lưu)
            SetMasteryCache(melee.id, mastery)
            -- Tiếp tục sang melee tiếp theo
        end
    end

    if allDone then
        _meleeAllDone = true
        SetTask("SubTask", "[AutoMelee] ✓ Tất cả melee đã hoàn thành!")
    end
...
-- =====================================================
--  ANTI AFK
--  Ngắt sự kiện Idled bằng cách giả lập click chuột phải
--  để tránh bị server kick sau 20 phút không tương tác
-- =====================================================
...
local function SetupAntiAfk()
    ...
    Player.Idled:Connect(function()
        ...
        print("[AntiAfk] ✓ Đã ngắt AFK kick")
    end)
end


-- =====================================================
--  AUTO BARTILO QUEST  (kích hoạt khi level >= 850, Sea 2)
--
--  Progress 0 → Nhận quest → giết Swan Pirate
--  Progress 1 → Giết Jeremy (hop server nếu không tìm thấy)
--  Progress 2 → Kích hoạt Bartilo Plates tại Dressrosa
-- =====================================================

local _bartiloQuestDone = false

-- Tìm plate chưa được kích hoạt (màu Sand yellow) theo thứ tự số
local function GetBartiloPlate()
    local ok, result = pcall(function()
...
local BARTILO_NPC_POS   = CFrame.new(-1561, 10, 1823)   -- Bartilo NPC
local BARTILO_PLATE_POS = CFrame.new(-1835, 10, 1679)   -- Khu vực plates

local function AutoBartiloQuest()
    if _bartiloQuestDone then return true end

    -- Chỉ chạy ở Sea 2
    local currentMap = workspace:GetAttribute("MAP")
    if currentMap ~= "Sea2" and currentMap ~= "Dungeons" then return false end
    ...
    if not ok then
        SetTask("SubTask", "[Bartilo] Không lấy được progress, thử lại...")
        task.wait(2)
        return false
    end
    ...
    -- ── Progress 0: Nhận quest → giết Swan Pirate ─────────────────────────
    if progress == 0 then
        -- Kiểm tra đã có quest Swan Pirate chưa
        local questVisible = false
        local questTitle   = ""
        pcall(function()
            questVisible = Player.PlayerGui.Main.Quest.Visible
            questTitle   = Player.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text
        end)

        if not questVisible or not string.find(questTitle, "Swan Pirate") then
            -- Chưa có quest → đến Bartilo NPC nhận quest
            SetTask("MainTask", "[Bartilo] Đến NPC Bartilo nhận quest...")
            if Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
                local dist = (BARTILO_NPC_POS.Position - Player.Character.HumanoidRootPart.Position).Magnitude
                if dist <= 15 then
                    task.wait(1)
                    pcall(function() CommF_:InvokeServer("StartQuest", "BartiloQuest", 1) end)
                else
                    topos(BARTILO_NPC_POS)
                    WaitTween(15)
                end
            end
        else
            -- Đã có quest → tìm và giết Swan Pirate
            SetTask("MainTask", "[Bartilo] Farm Swan Pirate...")
            local v = GetMobByName("Swan Pirate")
            if v then
                KillEnemy(v)
            else
                SetTask("SubTask", "[Bartilo] Swan Pirate chưa spawn → chờ...")
                local spawned = WaitForMobSpawn({"Swan Pirate"}, CFrame.new(-1400, 10, 1900), 30)
                if spawned then KillEnemy(spawned) end
            end
        end

    -- ── Progress 1: Giết Jeremy ───────────────────────────────────────────
    elseif progress == 1 then
        SetTask("MainTask", "[Bartilo] Tìm và giết Jeremy...")
        local jeremy = GetMobByName("Jeremy")
        if jeremy then
            KillMobByName("Jeremy")
        else
            -- Jeremy là boss unique, không có trong server → hop
            SetTask("SubTask", "[Bartilo] Không tìm thấy Jeremy → Hop server...")
            HopServers()
        end

    -- ── Progress 2: Kích hoạt Bartilo Plates ─────────────────────────────
    elseif progress == 2 then
        SetTask("MainTask", "[Bartilo] Kích hoạt Bartilo Plates...")

        -- Tween tới khu vực plates
        if Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
            local dist = (BARTILO_PLATE_POS.Position - Player.Character.HumanoidRootPart.Position).Magnitude
            if dist > 100 then
                topos(BARTILO_PLATE_POS)
                WaitTween(15)
            end
        end

        -- Liên tục kích hoạt plate tiếp theo cho đến khi xong
        local timeout = tick() + 60  -- tối đa 60s cho phase này
        while tick() < timeout do
            -- Re-check progress mỗi vòng
            local ok2, prog2 = pcall(function()
                return CommF_:InvokeServer("BartiloQuestProgress", "Bartilo")
            end)
            if ok2 and prog2 ~= 2 then break end  -- phase đã xong

            local plate = GetBartiloPlate()
            if plate then
                SetTask("SubTask", "[Bartilo] Kích hoạt plate: " .. plate.Name)
                pcall(function()
                    local hrp = Player.Character and Player.Character:FindFirstChildOfClass("BasePart")
                ...
                -- Không còn plate nào màu Sand yellow → phase 2 xong
                break
            end
        end

    -- ── Progress >= 3 hoặc giá trị khác: Quest hoàn thành ─────────────────
    else
        _bartiloQuestDone = true
        SetTask("MainTask", "[Bartilo] ✓ Bartilo Quest hoàn thành!")
        return true
    end

    -- Sau mỗi bước, re-check xem quest đã done chưa
    local ok3, finalProgress = pcall(function()
        return CommF_:InvokeServer("BartiloQuestProgress", "Bartilo")
    end)
    if ok3 and type(finalProgress) == "number" and finalProgress >= 3 then
        _bartiloQuestDone = true
        SetTask("MainTask", "[Bartilo] ✓ Bartilo Quest hoàn thành!")
        return true
    end
...
-- AntiAfk: kích hoạt ngay khi script load
SetupAntiAfk()
print("[Kaitun] ✓ AntiAfk → Online")

-- Loop 1: Auto Farm Level — ưu tiên: Saber(300) > Sea2(700) > Bartilo(850) > AutoMelee > Fruit > Farm
task.spawn(function()
    while task.wait(0.1) do
        ...
        -- Ưu tiên 2: Auto Travel Sea 2 khi đạt level 700 (và đang ở Sea 1)
        elseif currentLevel >= 700 and not _sea2Done and World < 2 then
            local sea2Ok, sea2Err = pcall(AutoTravelSea2)
            if not sea2Ok then
                SetTask("SubTask", "[Sea2] Lỗi: " .. tostring(sea2Err))
                task.wait(2)
            end

        -- Ưu tiên 3: Auto Bartilo Quest khi đạt level 850 (Sea 2)
        elseif currentLevel >= 850 and not _bartiloQuestDone and World == 2 then
            local bartiloOk, bartiloErr = pcall(AutoBartiloQuest)
            if not bartiloOk then
                SetTask("SubTask", "[Bartilo] Lỗi: " .. tostring(bartiloErr))
                task.wait(2)
            end

        -- Ưu tiên 4: Auto Fully Melee
        ...
        --   false     → không có gì cần làm với melee
            ...
            -- Ưu tiên 5: Collect fruit (kể cả khi đang farm mastery)
            ...
            -- Ưu tiên 6: Farm bình thường (bao gồm cả mastery farming)
            else
